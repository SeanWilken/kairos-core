...
export const INFRA_COMPONENTS = [
  ...
  { id: "reverse_proxy", label: "reverse_proxy", description: "Nginx / Caddy routing layer" },
];
...
export const STEP_DEFS = [
  ...
  { id: 3, label: "Secrets", description: "Key management strategy", icon: KeyRound },
  { id: 4, label: "Vector Store", description: "RAG configuration", icon: Database },
  { id: 5, label: "Preflight", description: "Config validation", icon: ShieldCheck },
  { id: 6, label: "Artifacts", description: "Generate config files", icon: PackageCheck },
  { id: 7, label: "Runtime Verify", description: "Run checks + docs", icon: Rocket },
  { id: 8, label: "Review", description: "Confirm & complete", icon: ClipboardCheck },
];
...
export const CHECK_OUTCOMES: Record<string, { status: CheckResult; message: string }> = {
  ...
  google_endpoint: { status: "pass", message: "generativelanguage.googleapis.com — 200 OK" },
  azure_openai_endpoint: { status: "warn", message: "Endpoint reachable — verify deployment name" },
...
export const STEP_HEADER: Record<number, { title: string; description: string }> = {
  ...
  3: { title: "Secrets Strategy", description: "Configure how credentials and sensitive values are managed." },
  4: { title: "Vector Store & RAG", description: "Optionally enable and configure a vector store for retrieval-augmented generation." },
  5: { title: "Preflight Validation", description: "Validate configuration consistency before generating artifacts." },
  6: { title: "Artifact Generation", description: "Generate deployment configuration files from your session." },
  7: { title: "Runtime Verify + Documents", description: "Run real runtime checks, then optionally bootstrap documents." },
  8: { title: "Review & Complete", description: "Review your configuration summary and confirm to complete setup." },
};
